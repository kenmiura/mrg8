c  Correction in mrg8dn: integer*4 iran(1) was not declared!  3-30-15   K.M.
c  Integer out put "iran1"is added only in mrg8dn.  3-24-15   KM  
...
c      10-6-06   KM
c
c   frt -KV9 source.f --> hardware 8-byte integer operations.
      program cortest
      parameter(NN=80000000)
      real*8  RAN1(NN),RAN2(NN)
      integer*4 N,NN,ISEED,MASK
      integer*4 IRAN1(NN),IRAN2(NN)
...
c
      ISEED=13579
      call mrg8dn(ISEED,RAN1,IRAN1,100)
      call clock(t0,2,2)
      call mrg8dn(ISEED,RAN1,IRAN1,N)
      call clock (t1,2,2)
      time=t1-t0
      call clock(t0,2,2)
      call mrg8dn(ISEED,RAN1,IRAN1,NN)
      call clock (t1,2,2)
      SUM=0.d0
      do i=1,NN
      SUM=SUM+RAN1(i)
...
c
      ...
      write(6,*)"First 1024 random numbers of the LAST run of mrg8dn:  real vs integer"
      do k=1,1024
      write(6,*) "(",k,")  ",RAN1(k),"  ",RAN2(k),"    ",IRAN1(k)
      enddo
c
      close(6)
      stop
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine mrg8dn(iseed,ran,iran,n)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
Copyright (C) 2006, Kenichi Miura,   All rights reserved. 
