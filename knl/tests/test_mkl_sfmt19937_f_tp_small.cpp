/*
 * MKL RNG -VSL_BRNG_SFMT19937_Fast- Thread Parallel
 *
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < ITER; ++i) {
        start = omp_get_wtime();
        mkl_rng_tp_small(ran, N, it, VSL_RNG_METHOD_UNIFORM_STD, VSL_BRNG_SFMT19937);
        end = omp_get_wtime();
        msec = (end - start) * 1000;
#ifdef DEBUG
        if (i == 0) {
    ...
    printf("EVALUATION, MKL_VSL_BRNG_SFMT19937_FAST_TP, %d, %d, %f, %f\n", tnum, N, mrng, ave_msec);
