/*
 * MRG8-Vectorized Random Generator Outer product version (non array)
 *
...
#pragma omp parallel
        {
            ...
            for (int j = 0; j < each_n; ++j) {
                ran[start + j] = m.mrg8_vec_outer(each_state);
            }
            if (tid == tnum - 1) {
                for (int j = 0; j < 8; ++j) {
    ...
    cout << "MRG8_VEC_OUTER_SINGLE_TP: " << mrng << " [million rng/sec], " << ave_msec << " [milli seconds]" << endl;
    printf("EVALUATION, MRG8_VEC_OUTER_SINGLE_TP, %d, %d, %f, %f\n", tnum, N, mrng, ave_msec);
