/*
 * MKL RNG -VSL_BRNG_MRG32K3A_Fast- Thread Parallel
 *
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < ITER; ++i) {
        start = omp_get_wtime();
        mkl_rng_tp_small(ran, N, it, VSL_RNG_METHOD_UNIFORM_STD, VSL_BRNG_MRG32K3A);
        end = omp_get_wtime();
        msec = (end - start) * 1000;
        // cout << msec << endl;
#ifdef DEBUG
    ...
    printf("EVALUATION, MKL_VSL_BRNG_MRG32K3A_FAST_TP, %d, %d, %f, %f\n", tnum, N, mrng, ave_msec);
